...
std::string utf8FromWide(const wchar_t *wide) {
  ...
  static_cast<void>(WideCharToMultiByte(CP_UTF8, 0, wide, -1, result.data(),
...
std::wstring wideFromUtf8(const std::string &utf8) {
  ...
  static_cast<void>(
      MultiByteToWideChar(CP_UTF8, 0, utf8.c_str(), -1, result.data(), needed));
  return result;
}
#endif
...
void RunWindowTask::contextChanged() {
  ...
  _platformService =
      dynamic_cast<vigine::platform::PlatformService *>(context()->service(
          "Platform", vigine::Name("MainPlatform"), vigine::Property::Exist));
  if (!_platformService) {
    _platformService =
        dynamic_cast<vigine::platform::PlatformService *>(context()->service(
            "Platform", vigine::Name("MainPlatform"), vigine::Property::New));
  }

  _graphicsService =
      dynamic_cast<vigine::graphics::GraphicsService *>(context()->service(
          "Graphics", vigine::Name("MainGraphics"), vigine::Property::Exist));
  if (!_graphicsService) {
    _graphicsService =
        dynamic_cast<vigine::graphics::GraphicsService *>(context()->service(
            "Graphics", vigine::Name("MainGraphics"), vigine::Property::New));
  }
  if (_graphicsService)
  ...
  _platformService->bindEntity(entity);
  {
    ...
    for (std::size_t windowIndex = 0; windowIndex < windows.size();
      ...
      for (auto *eventHandler : eventHandlers) {
        auto *windowEventHandler =
            dynamic_cast<WindowEventHandler *>(eventHandler);
        ...
        windowEventHandler->setKeyDownCallback(
            [this](const vigine::platform::KeyEvent &event) {
              std::cout << "[RunWindowTask::execute::lambda] keyCode="
                        << event.keyCode << ", scanCode=" << event.scanCode
        ...
        windowEventHandler->setCharCallback(
            [this](const vigine::platform::TextEvent &event) {
              onChar(event);
            });
        windowEventHandler->setWindowResizedCallback(
            [this, window](int width, int height) {
              onWindowResized(window, width, height);
            });
      }

      std::cout << "[RunWindowTask] Showing window " << (windowIndex + 1)
                << std::endl;
      window->setFrameCallback([this, window]() {
        ...
        if (_resizePending && _renderSystem) {
          ...
          const bool paused =
              now - _lastResizeEvent >= std::chrono::milliseconds(80);
          if (paused) {
            if (_pendingResizeWidth != _appliedResizeWidth ||
                _pendingResizeHeight != _appliedResizeHeight) {
              const bool resized = _renderSystem->resize(_pendingResizeWidth,
      ...
      if (showResult.isError())
        return showResult;
      std::cout << "[RunWindowTask] Window " << (windowIndex + 1)
                << " closed, continuing" << std::endl;
    }
  }
...
void RunWindowTask::onMouseButtonDown(vigine::platform::MouseButton button,
                                      int x, int y) {
  if (button == vigine::platform::MouseButton::Left) {
    vigine::Entity *picked =
        _renderSystem ? _renderSystem->pickFirstIntersectedEntity(x, y)
                      : nullptr;
  ...
  std::cout << "[RunWindowTask::onMouseButtonDown] button="
            << static_cast<int>(button) << ", x=" << x << ", y=" << y
            << std::endl;
  emitMouseButtonDownSignal(button, x, y);
}

void RunWindowTask::onMouseButtonUp(vigine::platform::MouseButton button, int x,
                                    int y) {
  static_cast<void>(x);
  static_cast<void>(y);
...
void RunWindowTask::onMouseWheel(int delta, int x, int y) {
  ...
  if (_objectDragActive) {
    ...
    _dragDistanceFromCamera =
        (std::max)(0.15f, _dragDistanceFromCamera * factor);
...
void RunWindowTask::updateCameraMovementKey(unsigned int keyCode,
                                            bool pressed) {
  if (!_renderSystem)
    return;
...
void RunWindowTask::onWindowResized(vigine::platform::WindowComponent *window,
...
void RunWindowTask::setTextEditorSystem(
...
bool RunWindowTask::handleClipboardShortcut(
  ...
  const bool ctrlPressed =
      (event.modifiers & vigine::platform::KeyModifierControl) != 0;
  if (!ctrlPressed)
    return false;
...
void RunWindowTask::setFocusedEntity(vigine::Entity *entity) {
  ...
  // visual effect.
  const bool isEditor =
      _textEditorSystem && _textEditorSystem->isEditorEntity(entity);
...
bool RunWindowTask::beginObjectDrag(vigine::Entity *entity, int x, int y) {
  ...
  if (!_renderSystem->screenPointToRayFromNearPlane(x, y, rayOrigin,
                                                    rayDirection))
    return false;
  ...
  _dragDistanceFromCamera =
      glm::dot(transform.getPosition() - rayOrigin, rayDirection);
...
bool RunWindowTask::ensureMouseRayEntity() {
  ...
  rc->setMesh(vigine::graphics::MeshComponent::createCube());
  rc->setShaderProfile(vigine::graphics::RenderComponent::ShaderProfile::Cube);
...
bool RunWindowTask::ensureMouseClickSphereEntity() {
  ...
  _mouseClickSphereEntity =
      entityManager->getEntityByAlias("MouseClickSphereEntity");
  if (!_mouseClickSphereEntity) {
  ...
  rc->setMesh(vigine::graphics::MeshComponent::createCube());
  rc->setShaderProfile(
      vigine::graphics::RenderComponent::ShaderProfile::Sphere);
  rc->setPickable(false);
...
void RunWindowTask::updateMouseRayVisualization(int x, int y) {
  ...
  if (!_renderSystem->screenPointToRayFromNearPlane(x, y, clickRayOrigin,
                                                    clickRayDirection))
    return;
  ...
  const glm::quat orientation =
      glm::rotation(glm::vec3(0.0f, 0.0f, 1.0f), rayDirection);
...
void RunWindowTask::updateMouseClickSphereVisualization(int x, int y) {
  ...
  const glm::vec3 sphereCenter =
      clickRayOrigin + clickRayDirection * kStartOffset;
