...
class TestStateExposed : public AbstractState
{
  public:
    ...
    TaskFlow *getTaskFlow() const { return AbstractState::getTaskFlow(); }
};

TEST(AbstractStateTest, check_isAbstract)
{
    EXPECT_TRUE((IsAbstract<AbstractState>)) << "AbstractState isn't an abstract";
}

TEST(AbstractStateTest, method_destructor)
{
    EXPECT_TRUE((HasMethod_destructor<AbstractState>)) << "AbstractState hasn't correct destructor";
}
...
{
    auto state = std::make_unique<TestStateExposed>();
    EXPECT_EQ(nullptr, state->getTaskFlow())
        << "getTaskFlow() should return nullptr when no task flow is set";

    auto taskFlow         = std::make_unique<TaskFlow>();
    TaskFlow *taskFlowPtr = taskFlow.get();
    state->setTaskFlow(std::move(taskFlow));
    EXPECT_EQ(taskFlowPtr, state->getTaskFlow())
        << "getTaskFlow() should return the correct task flow";
}
