...
void VulkanAPI::updateCameraDrag(int x, int y) {
  ...
  _cameraYaw -=
      static_cast<float>(deltaX) * kCameraControls.mouseLookSensitivity;
  _cameraPitch =
      std::clamp(_cameraPitch + static_cast<float>(deltaY) *
                                    kCameraControls.mouseLookSensitivity,
...
void VulkanAPI::setMoveForwardActive(bool active) {
  _moveForwardActive = active;
}

void VulkanAPI::setMoveBackwardActive(bool active) {
  _moveBackwardActive = active;
}
...
bool VulkanAPI::screenPointToRay(int x, int y, glm::vec3 &rayOrigin,
  ...
  const float height = static_cast<float>(_swapchainExtent.height);
  ...
  glm::mat4 projection =
      glm::perspective(glm::radians(60.0f), aspect, kCameraControls.nearPlane,
                       kCameraControls.farPlane);
  projection[1][1] *= -1.0f;

  const glm::vec3 forward = cameraForward(_cameraYaw, _cameraPitch);
  const glm::mat4 view = glm::lookAt(_cameraPosition, _cameraPosition + forward,
...
bool VulkanAPI::screenPointToRayFromNearPlane(int x, int y,
                                              glm::vec3 &rayOrigin,
                                              glm::vec3 &rayDirection) const {
  if (_swapchainExtent.width == 0 || _swapchainExtent.height == 0)
    return false;
...
bool VulkanAPI::hitTextEditorPanel(int x, int y) const {
  ...
  // Editor board is a plane centered at (0, 1.65, 1.2) with size 4.8 x 1.5 in
  // XY.
...
bool VulkanAPI::recreateSwapchainFromSurfaceExtent() {
  ...
  return recreateSwapchain(capabilities.currentExtent.width,
                           capabilities.currentExtent.height);
}
...
bool VulkanAPI::initializeInstance() {
  ...
  createInfo.ppEnabledLayerNames =
      _enableValidationLayers ? _validationLayers.data() : nullptr;
...
bool VulkanAPI::createSurface(void *nativeWindowHandle) {
  ...
  try {
    ...
    const VkResult createResult =
        vkCreateWin32SurfaceKHR(static_cast<VkInstance>(_instance.get()),
    ...
    if (!supportsPresent) {
      std::cerr << "Selected queue family does not support present"
                << std::endl;
      return false;
    }
  ...
  auto *metalLayer = static_cast<const CAMetalLayer *>(nativeWindowHandle);
  if (!metalLayer)
    return false;
  try {
    cleanupSwapchainResources();
    ...
    VkSurfaceKHR rawSurface = VK_NULL_HANDLE;
    const VkResult res =
        vkCreateMetalSurfaceEXT(static_cast<VkInstance>(_instance.get()),
                                &createInfo, nullptr, &rawSurface);
    if (res != VK_SUCCESS) {
      std::cerr << "Failed to create Metal surface, VkResult="
                << static_cast<int>(res) << std::endl;
      return false;
    }
    _surface = vk::SurfaceKHR(rawSurface);
    if (!_physicalDevice.getSurfaceSupportKHR(_presentQueueFamily, _surface)) {
      std::cerr << "Selected queue family does not support present"
                << std::endl;
      return false;
    }
  ...
  std::cerr << "Surface creation is not implemented for this platform"
            << std::endl;
  return false;
#endif
}
...
bool VulkanAPI::createSwapchain(uint32_t width, uint32_t height) {
  ...
  try {
    ...
    if (formats.empty() || presentModes.empty()) {
      std::cerr << "No swapchain formats or present modes available"
                << std::endl;
      return false;
    }
    ...
    for (const auto &image : _swapchainImages) {
      ...
      _swapchainImageViews.push_back(
          _device->createImageViewUnique(imageViewInfo));
    }
    ...
    auto vertShaderCode =
        loadBinaryFile({"build/bin/shaders/cube.vert.spv",
                        "bin/shaders/cube.vert.spv", "shaders/cube.vert.spv"});
    auto fragShaderCode =
        loadBinaryFile({"build/bin/shaders/cube.frag.spv",
                        "bin/shaders/cube.frag.spv", "shaders/cube.frag.spv"});
    ...
    auto pipelineResult = _device->createGraphicsPipelineUnique(
    ...
    // --- Text-voxel pipeline ---
    auto textVoxelVertCode = loadBinaryFile(
        {"build/bin/shaders/textvoxel.vert.spv",
         "bin/shaders/textvoxel.vert.spv", "shaders/textvoxel.vert.spv"});
    auto textVoxelFragCode = loadBinaryFile(
        {"build/bin/shaders/textvoxel.frag.spv",
    ...
    textVoxelVertInfo.pCode =
        reinterpret_cast<const uint32_t *>(textVoxelVertCode.data());
    auto textVoxelVertModule =
        _device->createShaderModuleUnique(textVoxelVertInfo);

    vk::ShaderModuleCreateInfo textVoxelFragInfo;
    textVoxelFragInfo.codeSize = textVoxelFragCode.size();
    textVoxelFragInfo.pCode =
        reinterpret_cast<const uint32_t *>(textVoxelFragCode.data());
    auto textVoxelFragModule =
        _device->createShaderModuleUnique(textVoxelFragInfo);
    ...
    auto textVoxelPipelineResult = _device->createGraphicsPipelineUnique(
        vk::PipelineCache{}, pipelineInfo);
    ...
    // --- Panel pipeline ---
    auto panelVertCode = loadBinaryFile({"build/bin/shaders/panel.vert.spv",
                                         "bin/shaders/panel.vert.spv",
                                         "shaders/panel.vert.spv"});
    auto panelFragCode = loadBinaryFile({"build/bin/shaders/panel.frag.spv",
                                         "bin/shaders/panel.frag.spv",
    ...
    panelVertInfo.pCode =
        reinterpret_cast<const uint32_t *>(panelVertCode.data());
    ...
    panelFragInfo.pCode =
        reinterpret_cast<const uint32_t *>(panelFragCode.data());
    ...
    std::array<vk::PipelineShaderStageCreateInfo, 2> panelShaderStages = {
        panelVertStage, panelFragStage};
    pipelineInfo.pStages = panelShaderStages.data();

    auto panelPipelineResult = _device->createGraphicsPipelineUnique(
        vk::PipelineCache{}, pipelineInfo);
    ...
    // --- Glyph pipeline ---
    auto glyphVertCode = loadBinaryFile({"build/bin/shaders/glyph.vert.spv",
                                         "bin/shaders/glyph.vert.spv",
                                         "shaders/glyph.vert.spv"});
    auto glyphFragCode = loadBinaryFile({"build/bin/shaders/glyph.frag.spv",
                                         "bin/shaders/glyph.frag.spv",
    ...
    glyphVertInfo.pCode =
        reinterpret_cast<const uint32_t *>(glyphVertCode.data());
    ...
    glyphFragInfo.pCode =
        reinterpret_cast<const uint32_t *>(glyphFragCode.data());
    ...
    std::array<vk::PipelineShaderStageCreateInfo, 2> glyphShaderStages = {
        glyphVertStage, glyphFragStage};
    pipelineInfo.pStages = glyphShaderStages.data();

    // Glyph pipeline uses per-instance vertex buffer: mat4 = 4 x vec4 at
    // binding 0.
    ...
    glyphVertexInputInfo.pVertexAttributeDescriptions =
        glyphInstanceAttribs.data();

    const auto *savedVertexInput = pipelineInfo.pVertexInputState;
    pipelineInfo.pVertexInputState = &glyphVertexInputInfo;

    auto glyphPipelineResult = _device->createGraphicsPipelineUnique(
        vk::PipelineCache{}, pipelineInfo);
    ...
    // --- Pyramid pipeline ---
    auto pyramidVertCode = loadBinaryFile({"build/bin/shaders/pyramid.vert.spv",
                                           "bin/shaders/pyramid.vert.spv",
                                           "shaders/pyramid.vert.spv"});
    auto pyramidFragCode = loadBinaryFile({"build/bin/shaders/pyramid.frag.spv",
                                           "bin/shaders/pyramid.frag.spv",
    ...
    pyramidVertInfo.pCode =
        reinterpret_cast<const uint32_t *>(pyramidVertCode.data());
    ...
    pyramidFragInfo.pCode =
        reinterpret_cast<const uint32_t *>(pyramidFragCode.data());
    ...
    std::array<vk::PipelineShaderStageCreateInfo, 2> pyramidShaderStages = {
        pyramidVertStage, pyramidFragStage};
    pipelineInfo.pStages = pyramidShaderStages.data();

    auto pyramidPipelineResult = _device->createGraphicsPipelineUnique(
        vk::PipelineCache{}, pipelineInfo);
    ...
    // --- Grid pipeline ---
    auto gridVertCode =
        loadBinaryFile({"build/bin/shaders/grid.vert.spv",
                        "bin/shaders/grid.vert.spv", "shaders/grid.vert.spv"});
    auto gridFragCode =
        loadBinaryFile({"build/bin/shaders/grid.frag.spv",
    ...
    gridVertInfo.pCode =
        reinterpret_cast<const uint32_t *>(gridVertCode.data());
    ...
    gridFragInfo.pCode =
        reinterpret_cast<const uint32_t *>(gridFragCode.data());
    ...
    std::array<vk::PipelineShaderStageCreateInfo, 2> gridShaderStages = {
        gridVertStage, gridFragStage};
    pipelineInfo.pStages = gridShaderStages.data();

    auto gridPipelineResult = _device->createGraphicsPipelineUnique(
        vk::PipelineCache{}, pipelineInfo);
    ...
    // --- Sun pipeline ---
    auto sunVertCode =
        loadBinaryFile({"build/bin/shaders/sun.vert.spv",
                        "bin/shaders/sun.vert.spv", "shaders/sun.vert.spv"});
    auto sunFragCode =
        loadBinaryFile({"build/bin/shaders/sun.frag.spv",
                        "bin/shaders/sun.frag.spv", "shaders/sun.frag.spv"});
    ...
    std::array<vk::PipelineShaderStageCreateInfo, 2> sunShaderStages = {
        sunVertStage, sunFragStage};
    pipelineInfo.pStages = sunShaderStages.data();

    auto sunPipelineResult = _device->createGraphicsPipelineUnique(
        vk::PipelineCache{}, pipelineInfo);
    ...
    auto sphereVertCode = loadBinaryFile({"build/bin/shaders/sphere.vert.spv",
                                          "bin/shaders/sphere.vert.spv",
                                          "shaders/sphere.vert.spv"});
    auto sphereFragCode = loadBinaryFile({"build/bin/shaders/sphere.frag.spv",
                                          "bin/shaders/sphere.frag.spv",
    ...
    sphereVertInfo.pCode =
        reinterpret_cast<const uint32_t *>(sphereVertCode.data());
    ...
    sphereFragInfo.pCode =
        reinterpret_cast<const uint32_t *>(sphereFragCode.data());
    ...
    std::array<vk::PipelineShaderStageCreateInfo, 2> sphereShaderStages = {
        sphereVertStage, sphereFragStage};
    pipelineInfo.pStages = sphereShaderStages.data();

    auto spherePipelineResult = _device->createGraphicsPipelineUnique(
        vk::PipelineCache{}, pipelineInfo);
    ...
    auto sdfGlyphVertCode = loadBinaryFile(
        {"build/bin/shaders/glyph_sdf.vert.spv",
         "bin/shaders/glyph_sdf.vert.spv", "shaders/glyph_sdf.vert.spv"});
    auto sdfGlyphFragCode = loadBinaryFile(
        {"build/bin/shaders/glyph_sdf.frag.spv",
    ...
    if (!sdfGlyphVertCode.empty() && !sdfGlyphFragCode.empty()) {
      ...
      _sdfDescriptorSetLayout =
          _device->createDescriptorSetLayoutUnique(sdfDslInfo);
      ...
      sdfVertModInfo.pCode =
          reinterpret_cast<const uint32_t *>(sdfGlyphVertCode.data());
      ...
      sdfFragModInfo.pCode =
          reinterpret_cast<const uint32_t *>(sdfGlyphFragCode.data());
      ...
      std::array<vk::PipelineShaderStageCreateInfo, 2> sdfStages = {
          sdfVertStage, sdfFragStage};

      // Vertex binding: binding 0, per-vertex, stride =
      // sizeof(GlyphQuadVertex).
      ...
      auto sdfResult = _device->createGraphicsPipelineUnique(
          vk::PipelineCache{}, sdfPipeInfo);
      ...
      std::cerr << "glyph_sdf shaders not found, SDF text rendering disabled"
                << std::endl;
    }
    ...
    for (const auto &imageView : _swapchainImageViews) {
      ...
      _swapchainFramebuffers.push_back(
          _device->createFramebufferUnique(framebufferInfo));
    }
    ...
    for (std::size_t i = 0; i < kMaxFramesInFlight; ++i) {
      _imageAvailableSemaphores.push_back(
          _device->createSemaphoreUnique(semaphoreInfo));
    ...
    std::cout << "Vulkan swapchain created successfully: " << extent.width
...
void VulkanAPI::setSdfGlyphData(std::vector<GlyphQuadVertex> vertices,
  ...
  stageAlloc.memoryTypeIndex = findMemoryType(
      stageReq.memoryTypeBits, vk::MemoryPropertyFlagBits::eHostVisible |
  ...
  // Allocate / update descriptor set if not done yet (needs
  // _sdfDescriptorSetLayout).
...
void VulkanAPI::setEntityModelMatrices(std::vector<glm::mat4> cubeMatrices,
  ...
  if (!glyphMatrices.empty()) {
    ...
    std::fill(_glyphInstanceUploadNeeded.begin(),
              _glyphInstanceUploadNeeded.end(), true);
  }
  _sphereEntityModelMatrices = std::move(sphereMatrices);
}

bool VulkanAPI::drawFrame() {
  if (!_device || !_swapchain || _inFlightFences.empty() ||
      _imageAvailableSemaphores.empty() || _renderFinishedSemaphores.empty())
    return false;
  ...
  try {
    ...
    // Clamp dt to avoid huge jumps after pauses, moving windows, or
    // breakpoints.
    ...
    if (acquireResult == vk::Result::eTimeout ||
        acquireResult == vk::Result::eNotReady)
      return false;
    ...
    if (imageIndex >= _commandBuffers.size() ||
        imageIndex >= _swapchainImages.size() ||
    ...
    const bool isInitialized = imageIndex < _imageInitialized.size() &&
                               _imageInitialized[imageIndex] != 0;

    vk::ImageMemoryBarrier toColorAttachment;
    toColorAttachment.oldLayout = isInitialized
    ...
    toColorAttachment.subresourceRange.aspectMask =
        vk::ImageAspectFlagBits::eColor;
    ...
    commandBuffer.pipelineBarrier(
        isInitialized ? vk::PipelineStageFlagBits::eBottomOfPipe
    ...
    const glm::vec3 sunlightDirection =
        glm::normalize(glm::vec3(-0.45f, -1.0f, -0.30f));
    ...
    const glm::vec3 skyColor = glm::mix(
        kSkyNight, kSkyDay, glm::clamp(sunElevation * 1.5f, 0.0f, 1.0f));

    std::array<vk::ClearValue, 2> clearValues{};
    clearValues[0].color = vk::ClearColorValue(
        std::array<float, 4>{skyColor.x, skyColor.y, skyColor.z, 1.0f});
    ...
    const glm::vec3 forward = cameraForward(_cameraYaw, _cameraPitch);
    const glm::mat4 view =
        glm::lookAt(_cameraPosition, _cameraPosition + forward,
                    glm::vec3(0.0f, 1.0f, 0.0f));
    glm::mat4 projection =
        glm::perspective(glm::radians(60.0f), aspect, kCameraControls.nearPlane,
                         kCameraControls.farPlane);
    ...
    // Draw ECS glyph-profile entities (instanced: one draw call for all
    // voxels).
    const auto glyphCount =
        static_cast<uint32_t>(_glyphEntityModelMatrices.size());
    if (glyphCount > 0) {
      ...
      if (needed > _glyphInstanceBufferCapacities[imageIndex]) {
        ...
        _glyphInstanceBuffers[imageIndex] =
            _device->createBufferUnique(bufInfo);

        const auto memReq = _device->getBufferMemoryRequirements(
            _glyphInstanceBuffers[imageIndex].get());
        vk::MemoryAllocateInfo allocInfo;
        allocInfo.allocationSize = memReq.size;
        allocInfo.memoryTypeIndex =
            findMemoryType(memReq.memoryTypeBits,
                           vk::MemoryPropertyFlagBits::eHostVisible |
                               vk::MemoryPropertyFlagBits::eHostCoherent);
        _glyphInstanceMemories[imageIndex] =
            _device->allocateMemoryUnique(allocInfo);
      ...
      if (_glyphInstanceUploadNeeded[imageIndex]) {
        void *data = _device->mapMemory(
            _glyphInstanceMemories[imageIndex].get(), 0, needed);
      ...
      commandBuffer.bindVertexBuffers(
          0, {_glyphInstanceBuffers[imageIndex].get()}, {offset});
    ...
    if (sdfVertCount > 0 && _sdfGlyphPipeline && _sdfAtlasReady) {
      ...
      if (sdfNeeded > _sdfGlyphVertexCapacities[imageIndex]) {
        ...
        _sdfGlyphVertexBuffers[imageIndex] =
            _device->createBufferUnique(sdfBufInfo);

        auto sdfMemReq = _device->getBufferMemoryRequirements(
            _sdfGlyphVertexBuffers[imageIndex].get());
        vk::MemoryAllocateInfo sdfAllocInfo;
        sdfAllocInfo.allocationSize = sdfMemReq.size;
        sdfAllocInfo.memoryTypeIndex =
            findMemoryType(sdfMemReq.memoryTypeBits,
                           vk::MemoryPropertyFlagBits::eHostVisible |
                               vk::MemoryPropertyFlagBits::eHostCoherent);
        _sdfGlyphVertexMemories[imageIndex] =
            _device->allocateMemoryUnique(sdfAllocInfo);
      ...
      if (_sdfGlyphUploadNeeded[imageIndex]) {
        void *sdfPtr = _device->mapMemory(
            _sdfGlyphVertexMemories[imageIndex].get(), 0, sdfNeeded);
        std::memcpy(sdfPtr, _sdfGlyphVertices.data(),
                    static_cast<std::size_t>(sdfNeeded));
        _device->unmapMemory(_sdfGlyphVertexMemories[imageIndex].get());
        _sdfGlyphUploadNeeded[imageIndex] = false;
      }

      commandBuffer.bindPipeline(vk::PipelineBindPoint::eGraphics,
                                 _sdfGlyphPipeline.get());
      pushConstants.modelMatrix = glm::mat4(1.0f);
      ...
      commandBuffer.bindDescriptorSets(vk::PipelineBindPoint::eGraphics,
                                       _sdfPipelineLayout.get(), 0, 1,
                                       &sdfDescSet, 0, nullptr);
      const vk::DeviceSize sdfOffset = 0;
      commandBuffer.bindVertexBuffers(
          0, {_sdfGlyphVertexBuffers[imageIndex].get()}, {sdfOffset});
    ...
    commandBuffer.pushConstants(_pipelineLayout.get(),
                                vk::ShaderStageFlagBits::eVertex |
                                    vk::ShaderStageFlagBits::eFragment,
                                0, sizeof(PushConstants), &pushConstants);
    commandBuffer.draw(768, 1, 0,
                       0); // UV-сфера: 16 lon x 8 lat x 2 tri x 3 vert
    ...
    const auto submitResult =
        _graphicsQueue.submit(1, &submitInfo, inFlightFence.get());
