#include "vigine/impl/actorhost/actorhost.h"
...
ActorHost::ActorHost(vigine::messaging::IMessageBus    &bus,
                                   vigine::core::threading::IThreadManager &threadManager)
...
std::unique_ptr<IActorMailbox> ActorHost::spawn(std::unique_ptr<IActor> actor)
{
    if (!actor)
    {
        return nullptr;
...
void ActorHost::stop(ActorId id)
{
    if (!id.valid())
    {
        return;
...
createActorHost(vigine::messaging::IMessageBus    &bus,
                vigine::core::threading::IThreadManager &threadManager)
{
    return std::make_unique<ActorHost>(bus, threadManager);
}
