#include "impl/messaging/buscontrolblock.h"
...
BusControlBlock::BusControlBlock() noexcept = default;
...
bool BusControlBlock::isAlive() const noexcept
{
    return _alive.load(std::memory_order_acquire);
}

void BusControlBlock::markDead() noexcept
{
    _alive.store(false, std::memory_order_release);
}

SlotAllocation
BusControlBlock::allocateSlot(AbstractMessageTarget *target)
{
    if (target == nullptr)
    {
...
void BusControlBlock::unregisterTarget(ConnectionId id) noexcept
{
    if (!id.valid())
    {
        return;
...
std::uint64_t BusControlBlock::registerSubscription(
...
void BusControlBlock::unregisterSubscription(std::uint64_t serial) noexcept
{
    if (serial == 0)
    {
        return;
...
std::vector<SubscriptionSlot> BusControlBlock::snapshotSubscriptions() const
{
...
BusControlBlock::LookupGuard
BusControlBlock::lookup(ConnectionId id) const noexcept
{
    if (!id.valid())
    {
        return {};
